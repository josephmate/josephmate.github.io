...
My phone's internet looked ok before I had to disconnect for the mobile hotspot.
...
The closest I got to diagnosing these kinds of issues was a lab from that course where we setup LAN routing and a DHCP server.
...
[The top answer](https://superuser.com/a/1858168/293310) says to assigned a static IP to fix the issue but does not explain how or why the laptop was given a new IP address.
...
This was what I initially thought too and quickly rejected because I'm not running a DHCP server.
This comment made me reconcider the theory
...
The message from 192.168.2.1 is (note: I changed the MAC address because I did not feel comfortable giving mine away):
```
...
It should be running a DHCP server
and I'm not sure how stop it.
As a result, the static IP workaround became a permanant fix.
...
I spent so much time installing it so I was way too lazy to buy and install another one.
...
Unfortunately for me and the other victims above, the DHCP server was running outside of this device setup window.
...
I do not have access to `/mnt/data/bin/wifi_start.s`.
I suspect that's where the [dhcpd](https://man.openbsd.org/dhcpd.8) is started.

# Open Questions

I'm only partially statisfied with the outcome because these answers just bring more questions than what I started with.
